...
class ServicesdetailsController extends Controller
{
    public function create(){
       ...
       public function store(Request $request){
        ...
        $maxSortCol = Servicesdetails::max('sort_col');
       
        // Set the sort_col for the new record
        $servicesdetails->sort_col = $maxSortCol ? $maxSortCol + 1 : 1; // If no records exist, start from 1
...
{
    $ids = $request->input('ids'); // Get the sorted Servicesdetails IDs from the frontend

    foreach ($ids as $index => $id) {
        // Update the sort_col field for each Servicesdetails based on the new index
