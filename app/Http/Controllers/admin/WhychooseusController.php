...
class WhychooseusController extends Controller
{
    public function create(){
       ...
       public function store(Request $request){
        ...
        $maxSortCol = Whychooseus::max('sort_col');
       
        // Set the sort_col for the new record
        $whychooseus->sort_col = $maxSortCol ? $maxSortCol + 1 : 1; // If no records exist, start from 1
    ...
    public function update(Request $request, $id)
{
    ...
    if ($request->hasFile('image')) {
        ...
        // Save as WebP for better compression & quality
        $imageCrop->save($path, 100, 'webp'); // 100 = highest quality

        // Delete the old image (optional)
        if ($whychooseus->image && file_exists(public_path('storage/' . $whychooseus->image))) {
            unlink(public_path('storage/' . $whychooseus->image));
        }
