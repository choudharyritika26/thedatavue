                    ...
                    <div class="blog-item bg-light rounded p-4" style="background-image: url(img/bg.png);">
                        <div class="mb-4">
                            ...
                            <div class="d-flex justify-content-between">
                                <p class="mb-0"><span class="text-dark fw-bold">On </span>{{$blog->date}}</p>
                                <p class="mb-0"><span class="text-dark fw-bold">By </span>{{$blog->name}}</p>
                            </div>
                        </div>
                        <div class="project-img">
                            <img src="{{ asset('storage/' . $blog->image) }}" class="img-fluid w-100 rounded" alt="Image">
                            <div class="blog-plus-icon">
                                <a href="{{ asset('storage/' . $blog->image) }}" data-lightbox="blog-1"
                                    class="btn btn-primary btn-md-square rounded-pill"><i class="fas fa-plus fa-1x"></i></a>
                            </div>
                        </div>
                        <div class="my-4">
                            <h4>{{$blog->heading}}</h4>    
                            {{-- {{ route('frontend.portfoliodetales', ['id' => $portfolioItem->id]) }} --}}
                        </div>   
                        <a class="btn btn-primary rounded-pill py-2 px-4" href="{{ route('frontend.blogdetail' , ['id' => $blog->id]) }}">Explore More</a>
                    </div>
                </div>
                @endforeach
