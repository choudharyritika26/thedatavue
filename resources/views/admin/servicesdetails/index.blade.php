...
<main id="main" class="main">
    ...
    <div class="pagetitle">
        {{-- <h1 style="margin-top: 20px">Services Details Management</h1> --}}
        <span>
            {{-- @if ($servicesdetails->isEmpty()) --}}
                <a href="{{ route('add-servicesdetails') }}"><button class="btn btn-primary">Add</button></a>
            {{-- @endif --}}
        </span>
    </div><!-- End Page Title -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table text-center">
                                ...
                                <tbody>
                                    @foreach ($servicesdetails as $index => $item)
                                    <tr class="sortable-item" data-id="{{ $item->id }}">
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $item->ServicescatagriesName->heading ?? '' }}</td>
                                        <td>{{ $item->heading }}</td>
                                        <td>{!! html_entity_decode($item->description) !!}</td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center">
                                                <!-- Edit Button -->
                                                <a href="{{ route('edit-servicesdetails', $item->id) }}"
                                                    class="btn btn-info btn-sm mx-2" title="Edit">
                                                    <i class="fa fa-edit" style="font-size:20px;"></i>
                                                </a>

                                                <form method="POST" action="{{ route('destroy-servicesdetails', $item->id) }}"
    ...
    $(document).ready(function() {
        ...
        $("tbody").sortable({
            ...
            update: function(event, ui) {
                ...
                $.ajax({
                    url: "{{ route('update-servicesdetails-order') }}",  // Define your route here   
                    method: "POST",
                    data: {
                        ids: sortedIDs,
